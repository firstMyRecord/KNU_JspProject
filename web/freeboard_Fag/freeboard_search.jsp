...
<FORM class="form-inline" method=post name=search action="/freeboard_Fag/freeboard_search.jsp">
<div class="container">
 	<div class="row form-group">
		[<A href="/freeboard_Fag/freeboard_listNotice.jsp">자유 게시판(일반모드)</A>]
	</div>
	<div class="row form-group">
...
// Connection con= null;
 ...
 try {
  ...
  String sql = "select * from Fag " + cond;
  ...
  if (!(rs.next()))  {
   ...
   for(int j=startrow;j<=endrow;j++) {
    ...
    String clink = "<A href=/freeboard_Fag/freeboard_read.jsp?id=" + keyid.elementAt(j) ;
...
//추가    
 ...
 if (wheregroup > 1) {
  out.print("[<A href=/freeboard_Fag/freeboard_search.jsp?gogroup=1");
  out.print("&stype="+ what+"&sval=" + val +">처음</A>]");
  out.print("[<A href=/freeboard_Fag/freeboard_search.jsp?gogroup="+priorgroup);
 ...
 if (name.size() !=0) {
  for(int jj=startpage; jj<=endpage; jj++) {
   ...
   out.print("[<A href=/freeboard_Fag/freeboard_search.jsp?go="+jj);
   out.print("&stype="+ what+"&sval=" + val +">" + jj + "</A>]") ;
   }
  }
 }
 if (wheregroup < totalgroup) {
  out.print("[<A href=/freeboard_Fag/freeboard_search.jsp?gogroup="+ nextgroup);
  out.print("&stype="+ what+"&sval=" + val +">다음</A>]");
  out.print("[<A href=/freeboard_Fag/freeboard_search.jsp?gogroup="+totalgroup);
