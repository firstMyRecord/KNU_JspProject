...
//추가 끝
 ...
 if (wheregroup > 1) {
  out.println("[<A href=/freeboard/freeboard_list.jsp?gogroup=1>처음</A>]");
  out.println("[<A href=/freeboard/freeboard_list.jsp?gogroup="+priorgroup +">이전</A>]");
 } else {
  out.println("[처음]") ;
  out.println("[이전]") ;
 }
 if (name.size() !=0) {
  for(int jj=startpage; jj<=endpage; jj++) {
   if (jj==where)
    ...
    out.println("[<A href=/freeboard/freeboard_list.jsp?go="+jj+">" + jj + "</A>]") ;
   }
  }
  if (wheregroup < totalgroup) {
   out.println("[<A href=/freeboard/freeboard_list.jsp?gogroup="+ nextgroup+ ">다음</A>]");
   out.println("[<A href=/freeboard/freeboard_list.jsp?gogroup="+ totalgroup + ">마지막</A>]");
  } else {
   out.println("[다음]");
   out.println("[마지막]");
  }
