...
<P>
<P align=center><FONT color=#0000ff face=굴림 size=5><STRONG>질문하기</STRONG></FONT></P>
<P>
 <CENTER>
<TABLE class="Fag">
 <thead>
...
//  Connection con= null;
 ...
 try {
  ...
  String sql = "select * from GNA order by" ;
  ...
  if (!(rs.next()))  {
   ...
   for(int j=startrow;j<=endrow;j++) {
    ...
    if (stepi > 0 ) {
     ...
     out.print("<A href=/freeboard_GNA/freeboard_read.jsp?id=");
...
//추가 끝
 ...
 if (wheregroup > 1) {
  out.println("[<A href=/freeboard_GNA/freeboard_listFag.jsp?gogroup=1>처음</A>]");
  out.println("[<A href=/freeboard_GNA/freeboard_listFag.jsp?gogroup="+priorgroup +">이전</A>]");
 } else {
  out.println("[처음]") ;
  out.println("[이전]") ;
 }
 if (name.size() !=0) {
  for(int jj=startpage; jj<=endpage; jj++) {
   if (jj==where)
    ...
    out.println("[<A href=/freeboard_GNA/freeboard_listFag.jsp?go="+jj+">" + jj + "</A>]") ;
   }
  }
  if (wheregroup < totalgroup) {
   out.println("[<A href=/freeboard_GNA/freeboard_listFag.jsp?gogroup="+ nextgroup+ ">다음</A>]");
   out.println("[<A href=/freeboard_GNA/freeboard_listFag.jsp?gogroup="+ totalgroup + ">마지막</A>]");
  } else {
   out.println("[다음]");
   out.println("[마지막]");
  }
 ...
 <br><br>
 <FORM class="form-inline" method="post" name="msgsearch" action="/freeboard_GNA/freeboard_search.jsp">
  <div class="container_div">
   ...
   <br>
   <div align=right><A href="/freeboard_GNA/freeboard_writeFag.jsp"><img src="image/write.gif" class="img-fluid"></A></div>
  </div>
 </FORM>
